...
export const githubOAuthService = {
    ...
    async getUserProfile(code: string, _state: string) {
        ...
        const profile = await profileRes.json() as { id: number; login: string; email: string; name: string; email_verified: boolean };
        ...
        if (!email) {
            ...
            const emails = await emailRes.json() as Array<{ primary: boolean; email: string }>;
